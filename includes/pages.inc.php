...
class bodyClass {
    ...
    public function showResources() {
        ...
        <div class="body-content">
            ...
            while ($checkRow = $stmt->fetch()) {
                ...
                echo '<div class="padder"><a id="supporters" href="http://' . $resourceLink . '" target="_blank">' . $resourceName . '</a></div>';
            }
            ?>
    ...
    public function showSupport() {
        ...
        echo '<div class="body-content">';
        echo '<div class="space"></div>';
        echo '<center><h1>Support</h1></center><br><br>';

        if ($stmt = $this->dbConnection->prepare("SELECT aboutID, aboutDescription FROM about WHERE aboutID=? ")) {

            $stmt->bind_param("i", $aboutID);
            $stmt->execute();
            $stmt->bind_result($aboutID, $aboutDescription);
            $stmt->fetch();
